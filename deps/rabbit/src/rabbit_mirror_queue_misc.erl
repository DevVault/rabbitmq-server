   ...
   {classic_mirrored_queues,
    #{deprecation_phase => permitted_by_default,
      ...
      #{when_permitted =>
        ...
        "RabbitMQ version and they will be removed from RabbitMQ 4.0.0.\n"
        "To continue using classic mirrored queues when they are not "
        "permitted by default, set the following parameter in your "
        "configuration:\n"
        "    \"deprecated_features.permit.classic_mirrored_queues = true\"\n"
...
are_cmqs_used(_) ->
    ...
    HAConfigured = has_ha_policies(Policies),

    OpPolicies = rabbit_policy:list_op(),
    OpHAConfigured = has_ha_policies(OpPolicies),

    HAConfigured orelse OpHAConfigured.
